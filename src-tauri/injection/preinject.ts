...
async function init() {
	...
	await invoke("start_streamer_mode_watcher");

	const plugins = await invoke("load_plugins").catch((e) =>
	...
	safemodeTimer(document.querySelector("#safemode") as HTMLDivElement);

	updateOverlay({
		subtitle: `Made with ❤ by SpikeHD - v${version}`,
...
async function handleThemeInjection() {
	...
	if (
		!window.__DORION_CONFIG__?.theme ||
		window.__DORION_CONFIG__?.theme === "none"
	)
		return "";
...
async function displayLoadingTop() {
	...
	const loadingContainer = document.createElement(
		"div",
