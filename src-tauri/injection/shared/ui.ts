...
import { setMaximizeIcon } from "./window";
...
export async function applyExtraCSS() {
	...
	if (
		window.__DORION_CONFIG__.blur !== "none" &&
		window.__DORION_CONFIG__.blur_css
	) {
		style.innerHTML += `
      * {
...
async function initTopBarEvents() {
	...
	topclose.addEventListener("click", () => getCurrent().close());
	topmin.addEventListener("click", () => getCurrent().minimize());
	topmax.addEventListener("click", () => getCurrent().maximize());
}
