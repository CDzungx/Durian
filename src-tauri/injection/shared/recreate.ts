...
export function proxyFetch() {
	...
	window.fetch = async (url, options) => {
		...
		const discordReg =
			/https?:\/\/(?:[a-z]+\.)?(?:discord\.com|discordapp\.com)(?:\/.*)?/g;
		...
		if (options?.body) {
			options.body = isJson(String(options.body))
				? http.Body.json(options.body)
				: typeof options.body === "string"
					? http.Body.text(options.body)
					: http.Body.bytes(options.body);
		}
