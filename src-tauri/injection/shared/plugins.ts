export async function ensurePlugins() {
	...
	if (
		window.__DORION_CONFIG__?.client_plugins != null &&
		!window.__DORION_CONFIG__?.client_plugins
	) {
		console.log(
			"[Ensure Plugins] Client plugins are disabled. I hope you know what you are doing!",
		);
		return;
	}
	...
	const promises = [
		...Object.entries(requiredPlugins).map(async ([name, url]) => {
			...
			try {
				...
				const fn = new Function("shelter", `return ${pluginStr}`);
