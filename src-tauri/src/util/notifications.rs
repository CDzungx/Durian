...
use tauri::{Manager};
use tauri_plugin_notification::{NotificationBuilder, NotificationData, NotificationExt};
...
fn send_notification_internal(win: tauri::WebviewWindow, title: String, body: String, icon: String) {
    ...
    // Chain builder methods to configure notification
    ...
    // Send the notification using the app handle
...
pub unsafe fn set_notif_icon(window: &tauri::WebviewWindow, amount: i32) {
    ...
    use windows::Win32::{
        ...
        UI::{
            ...
            WindowsAndMessaging::{CreateIconFromResourceEx, LR_DEFAULTCOLOR, IMAGE_FLAGS},
        },
    };
    ...
    CoInitialize(Some(std::ptr::null())).ok().expect("Failed to initialize COM library");
    ...
    let presbits = std::slice::from_raw_parts(ico.0, ico.1);
    let ficon = BOOL(1);
    let flags = IMAGE_FLAGS(0);
    ...
    taskbar_list
        .SetOverlayIcon::<windows::Win32::Foundation::HWND, _, _>(
            hwnd.into(),
            hicon,
            None,
        )
